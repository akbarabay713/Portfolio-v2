...
const ProjectCard = ({ project, viewMode = "grid" }: ProjectCardProps) => {
  ...
  return (
    <Card
      interactive
      className={cn(
        "group h-full gap-4 p-4 shadow-card",
        isList ? "flex flex-col md:flex-row md:items-center md:gap-6" : "flex flex-col",
      )}
    >
      <div
        className={cn(
          "relative aspect-4/3 overflow-hidden rounded-md bg-elevated",
      ...
      >
        ...
        {live && (
          <a
            ...
            className="absolute bottom-3 right-3 flex h-12 w-12 translate-y-3 items-center justify-center rounded-full bg-accent text-accent-contrast opacity-0 shadow-dialog transition-all duration-300 hover:scale-105 group-hover:translate-y-0 group-hover:opacity-100"
          >
      ...
      <div className="flex flex-1 flex-col px-1">
        <h3 className="mb-2 text-base font-bold tracking-tight text-content">
          {title}
        </h3>
        <p
          className={cn(
